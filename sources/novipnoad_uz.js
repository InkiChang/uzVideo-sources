...
//@remark:移植自 XPTV novipnoad.js (novipnoad.net)，HTML刮削源
//@type:101
//@order: C
// ignore
...
// ============================================================
// NO視頻 uzVideo 扩展
// 移植自 XPTV xptv-extensions/js/novipnoad.js
// 站点: novipnoad.net (HTML 刮削 + vkey/RC4 播放解密)
//
// 接口映射:
...
if (typeof $print === 'undefined') { var $print = function(s) { try { console.log(s) } catch(e) {} } }
const $utils = {
    toastInfo: (msg) => { try { console.log('[novipnoad]', msg) } catch(e) {} },
    openSafari: (url, ua) => { try { console.log('[novipnoad] openSafari(ignored):', url) } catch(e) {} },
}
// ⚠️ novipnoad.net 有 Cloudflare("Just a moment"), 单薄请求头会被拦
// 与同仓库 saohuo_uz.js 一致的修复: polyfill 层注入完整浏览器头
// 注意: 原始 UA(iPhone) 在 opts.headers 中, 会被 BROWSER_HEADERS 覆盖为 Chrome 桌面UA
// 页面为响应式结构, 同一套 cheerio 选择器对桌面/移动版 DOM 通用
...
// ============================================================
// 以下为 XPTV 原始代码 (verbatim, 仅删除首行 cheerio 冲突声明)
// ============================================================
...
// ============================================================
// uzVideo Wrapper (getClassList/getVideoList/getVideoDetail/
//                  getVideoPlayUrl/searchVideo)
// ============================================================
...
async function getClassList(args) {
    ...
    try {
        ...
        for (var i = 0; i < tabs.length; i++) {
            var c = new VideoClass()
            c.type_id = tabs[i].ext.url
            c.type_name = tabs[i].name
            c.has_subclass = false
            list.push(c)
        }
        backData.data = list
    } catch (e) {
...
async function getVideoList(args) {
    ...
    try {
        ...
        var ext = jsonify({ url: url, page: page })
        ...
        for (var i = 0; i < cards.length; i++) {
            ...
            vd.vod_id = (c.ext && c.ext.url) ? c.ext.url : String(c.vod_id || '')
...
async function getVideoDetail(args) {
    ...
    try {
        ...
        for (var i = 0; i < lists.length; i++) {
            var line = lists[i]
            var lineName = line.title || ('線路' + (i + 1))
            var tracks = line.tracks || []
            var eps = []
            for (var j = 0; j < tracks.length; j++) {
                var t = tracks[j]
                var epName = t.name || ('第' + (j + 1) + '集')
                // ⚠️ 本站集数 ext = {vid,pkey,ref}, getPlayinfo 需要三者
                // 把整个 ext JSON 塞入 vod_play_url 作为载体, getVideoPlayUrl 里解码
                var epUrl = encodeURIComponent(JSON.stringify(t.ext || {}))
...
async function getVideoPlayUrl(args) {
    ...
    try {
        ...
        // 解码 getVideoDetail 时编码的 {vid,pkey,ref}
        var extObj = null
        try {
            extObj = JSON.parse(decodeURIComponent(url))
        } catch (e) { /* fallback: maybe plain url */ }
        var ext = extObj ? jsonify(extObj) : jsonify({ url: url })
        var result = JSON.parse(await getPlayinfo(ext))
        var urls = result.urls || []
        if (urls.length > 0) {
            backData.data = urls[0]
            // 本站返回对象格式 headers: {UA,Referer,Origin}
            if (result.headers) {
                backData.headers = result.headers
            }
        } else {
